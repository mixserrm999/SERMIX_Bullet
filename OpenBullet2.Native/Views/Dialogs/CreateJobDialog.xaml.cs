        ...
        private void CreateJob(JobType type)
        {
            ...
            switch (type)
            {
                case JobType.MultiRun:
                    new MainDialog(new MultiRunJobOptionsDialog(null, onAccept), "Create Multi Run Checker", 800, 600).ShowDialog();
                    break;

                case JobType.ProxyCheck:
                    new MainDialog(new ProxyCheckJobOptionsDialog(null, onAccept), "Create Proxy Checker").ShowDialog();
                    break;
            }
